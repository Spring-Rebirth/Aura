...
export async function getPopularPosts() {
    return await fetchData(
        ...
        config.videosCollectionId,
        [
            Query.greaterThanEqual("SavedCount", 1),
            Query.orderDesc('SavedCount'),
            Query.limit(8)
        ],
        'Failed to popular latest posts'
    );
}

// 搜索帖子
// const segmentit = useDefault(new Segment());  中文分词插件
export async function searchPosts(query) {
    try {
        // // 使用 js-jieba 对查询文本进行分词
        // const segments = segmentit.doSegment(query);
        // console.log(segments);
        // const searchQuery = segments.map(segment => segment.w).join('');
        // console.log("搜索字符串:", searchQuery);
...
export const syncDataToBackend = async (playData) => {
    const unsyncedData = Object.entries(playData.current).filter(
        ([, value]) => !value.synced
    );
    ...
    try {
        // 遍历每个未同步的数据并更新后端
        for (const [videoId, { count }] of unsyncedData) {
            ...
            console.log(`视频 ${videoId} 的播放次数已更新为 ${count}`);

            // 标记为已同步
            playData.current[videoId].synced = true;
        }

        // 更新本地存储
        await AsyncStorage.setItem('playData', JSON.stringify(playData.current));
    } catch (error) {
