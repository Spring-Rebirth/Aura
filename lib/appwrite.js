...
export const registerUser = async (email, password, username, setTempUserId) => {
    try {
        // 创建新账户
        const user = await account.create(ID.unique(), email, password, username);
        setTempUserId(user.$id); // 保存未注册成功的用户 ID
        ...
        const Verification = await account.createVerification('https://cloud.appwrite.io/v1');
        console.log('验证邮件已发送:', Verification);
        // 返回新创建的用户文档
        return { Verification };
    } catch (error) {
...
//     console.error('用户未验证退出了注册页  userId: ', userId);
//     console.warn('需要到Appwrite控制台手动删除');
//     Alert.alert(
//         'You exited during the verification phase.',
//         'To re-register the same account, please contact the App administrator to clear it.',
//         'User ID: ', userId
//     );
// };
